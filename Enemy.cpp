	...
	if (OtherActor)
	{
		AMain* Main = Cast<AMain>(OtherActor);
		{
			if (Main)
			{
				...
				/** If we move out of aggro sphere, hide the health bar again */
				Main->SetHasCombatTarget(false);
				
				Main->UpdateCombatTarget();

				/** Sets the enemy to idle when we leave the aggro sphere */
	...
	if (OtherActor && Alive())
	{
		AMain* Main = Cast<AMain>(OtherActor);
		{
			if (Main)
			{
				...
				/** This is so the enemy doesn't attack us right away */
